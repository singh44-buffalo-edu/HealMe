...
- ⚠️ **Binary storage lives on the `medplum-binary` named volume** (added 2026-07-21 after a wipe): `file:./binary/` holds uploaded documents, generated PDFs AND deployed bot code. Before the volume existed, every container recreation destroyed it — symptom: every bot fails `"File not found"` at `FileSystemStorage.readFile`, and reads of old Binaries 500. Recovery: `make bots` (bot code), but document content is gone unless `make backup` ran. Never remove that volume mapping.
- **`oauth2/userinfo` runs under the caller's AccessPolicy**: it reads the caller's own profile resource, so a client-credentials session needs read access to its own `ClientApplication` (bootstrap.py adds a `_id`-scoped entry). Without it userinfo 403s and the ai-service auth gate rejects valid service tokens with "Session expired or invalid".
